...
class SnsSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['directions'] = [
      '#markup' => 'All topics should be defined in your site\'s settings file.
        Each topic will show up here, and you can go through each of the bundles
        and select the node operations (insert/update/delete) you want
        to enable.',
    ];

    $form['enabled_sns_entities'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Enabled SNS Entities'),
      '#tree' => TRUE,
    ];
    ...
    foreach ($topics as $topic => $arn) {
      $form['enabled_sns_entities'][$topic] = [
        '#type' => 'details',
        '#title' => $this->t('Topic Settings for @topic', ['@topic' => $topic]),
        '#tree' => TRUE,
        '#open' => FALSE,
      ];
      ...
      foreach ($entity_types as $entity_type) {
        ...
        $form['enabled_sns_entities'][$topic][$entity_type_id] = [
          '#type' => 'fieldset',
          '#title' => $this->t('@entity_type_id Bundles', [
   ...
   *
   * @return array
   *   An array of strings whose values represent the different bundles within
   *   the entity type.
   */
